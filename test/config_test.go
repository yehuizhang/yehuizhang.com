...
import (
	...
	c "yehuizhang.com/go-webapp-gin/src/config"
)

func Test_InitConfig_OnSuccess(t *testing.T) {
	observedZapCore, observedLogs := observer.New(zap.InfoLevel)
	observedLogger := zap.New(observedZapCore).Sugar()
	config, err := c.InitConfig(&flags.FlagParser{Env: "local", ConfigName: ".env", ConfigPath: ".", ConfigType: "env"}, observedLogger)
...
func Test_InitConfig_Invalid_File_Name(t *testing.T) {
	...
	observedLogger := zap.New(observedZapCore).Sugar()
	_, err := c.InitConfig(&flags.FlagParser{Env: "", ConfigName: ".invalid", ConfigPath: ".", ConfigType: "env"}, observedLogger)
...
func Test_InitConfig_Incorrect(t *testing.T) {
	observedZapCore, _ := observer.New(zap.InfoLevel)
	observedLogger := zap.New(observedZapCore).Sugar()
	_, err := c.InitConfig(&flags.FlagParser{Env: "local", ConfigName: ".env", ConfigPath: ".", ConfigType: "json"}, observedLogger)
